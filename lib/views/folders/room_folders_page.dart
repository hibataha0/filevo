...
class _RoomFoldersPageState extends State<RoomFoldersPage> {
  ...
  bool isLoading = true;
  Map<String, Map<String, dynamic>> _folderDetailsCache = {}; // ✅ Cache لتفاصيل المجلدات
  ...
  Future<void> _loadFoldersDetails() async {
    ...
    final futures = folders.map((folder) async {
      try {
        ...
        try {
          details = await folderController.getSharedFolderDetailsInRoom(folderId: folderId);
        } catch (e) {
          // ✅ إذا فشل، حاول جلب تفاصيل المجلد العادية
          try {
            details = await folderController.getFolderDetails(folderId: folderId);
          } catch (e2) {
  ...
  Widget _buildFoldersList() {
    ...
    final displayFolders = folders.map((folder) {
      ...
      final filesCount = cachedDetails?['filesCount'] ?? folderData['filesCount'] ?? 0;
    ...
    try {
      ...
      if (mounted) {
        if (success) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(roomController.errorMessage ?? '❌ فشل إزالة المجلد من الغرفة'),
